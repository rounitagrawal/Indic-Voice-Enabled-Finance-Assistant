...
def ASR_config_call(language: str) -> tuple[str, str]:
    ...
    service_id = json_object["pipelineResponseConfig"][0]["config"][0]["serviceId"]
    inference_api_key = json_object["pipelineInferenceAPIEndPoint"]["inferenceApiKey"][
        "value"
    ]
...
def ASR_call(language: str, audio_64: str) -> str:
    ...
    response = requests.post(_INFERENCE_URL, headers=_inference_headers(), data=payload)
    response.raise_for_status()
